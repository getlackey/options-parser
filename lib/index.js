...
Obj.prototype.makeArray = function (convertAll) {
    var self = this;

    Object.keys(self).forEach(function (key) {
        var item = self[key];
        self[key] = ((convertAll || item.indexOf(',') > -1) ? item.split(',') : item);
    });
...
/**
 * Process file include oprations
 * @param string} opts
 *
 * @example
 *  /path/path/path/file.json
 *  /path/path/file.js
 *  /path/path/file.js#function
 *  /path/path/file.js#function?argument=1&argument2
 *  /path/path/file.js?argument=1&arguments2
 */
function fileInclude(opts) {
    ...
    if (file.indexOf('?') !== -1) {
        elems = file.split('?');
        file = elems.shift();
        args = querystring.parse(elems.join('?'));
    }

    if (file.indexOf('#') !== -1) {
        elems = file.split('#');
        file = elems.shift();
        func = elems[0] ? elems[0].replace(/^\s+|\s+$/, '') : null;
    }

    result = require(path.join(process.cwd(), file));
...
module.exports = function optionsParser(opts) {
    ...
    if (typeof opts === 'string' && opts.indexOf('/') === 0) {
        ...
        if (String(typeof window) === 'undefined') { //isn't browser?
            opts = fileInclude(opts);
        } else {
